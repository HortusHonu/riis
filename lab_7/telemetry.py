...
async def position_logger(drone):
    async for pos in drone.telemetry.position():
        print(f"{datetime.now().strftime('%H:%M:%S')} [POS] Lat: {pos.latitude.deg:.6f}, Lon: {pos.longitude_deg:.6f}, Alt: {pos.relative_altitude_m:.2f}m")

async def attitude_logger(drone):
    async for att in drone.telemetry.euler():
        print(f"{datetime.now().strftime('%H:%M:%S')} [ATT] Roll: {att.roll_deg:.2f}, Pitch: {att.pitch_deg:.2f}, Yaw: {att.yaw_deg}")

async def battery_logger(drone):
    async for b in drone.telemetry.battery():
        print(f"{datetime.now().strftime('%H:%M:%S')} [BAT] Percentage: {b.remaining_percent:.0f}, Voltage: {b.voltage_v}")

async def speed_logger(drone):
    async for s in drone.telemetry.velocity_ned():
        speed = math.sqrt(s.north_m_s**2 + s.east_m_s**2)
        print(f"{datetime.now().strftime('%H:%M:%S')} [SPD] Speed: {speed:.2f} m/s")
